...
class GPUPage extends StatefulWidget{
  ...
  GPUHPage createState() => GPUHPage(data: 'Test');//, myCPU: myCPU);

}

class GPUHPage extends State<GPUPage> {
  final String data;
  //CPU myCPU;
  GPUHPage({required this.data});
  ...
  Future<List<dynamic>> loadJsonData() async {
    String jsonString = await rootBundle.loadString('assets/video-card.json');
  ...
  Future<void> _navigateAndSave(BuildContext context, Map<String, dynamic> chosenOne) async{
    final result = await Navigator.push(
        context,
        MaterialPageRoute(builder: (context) => GPUInfo(data: chosenOne)));
    save_Data = "$result";
                ...
                onChanged: (value) {
                  setState(() {
                    searchItem = _items.where((element) => element['name'].contains(value)
                        || element['chipset'].contains(value))
                        .toList();
                  });
                },
                          ...
                          child: Row(
                            children: [
                              ...
                              Text(
                                  ...
                                  _items[index]['name'] + " " + _items[index]['chipset']
                                      : searchItem[index]['name']+ " " + searchItem[index]['chipset'],
...
class GPUInfo extends StatelessWidget{
  Map<String, dynamic> data;
  GPUInfo({Key? key, required this.data}) : super(key : key);
          ...
          child: Column(
            ...
            children: [
              ...
              IconButton(
                ...
                color: Colors.green,
                onPressed: (){
                  Navigator.pop(context, data['name'] + " " + data["chipset"]);
                },
              ),
            ],
          ),
